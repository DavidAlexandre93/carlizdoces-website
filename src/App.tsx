...
import { BRL, EASTER_ENABLED_DEFAULT, PHONE, fetchCatalog } from './catalog-data';
import { useQuery } from '@tanstack/react-query';
import type { CatalogData, Product } from './catalog-data';
  ...
  return (
    <section className="rounded-3xl bg-gradient-to-r from-carliz-pink to-fuchsia-400 p-6 text-white shadow-lg md:p-10">
      ...
      <p className="max-w-2xl text-white/95">
        Doces artesanais para momentos especiais. Monte seu pedido e envie direto no WhatsApp em um clique.
      </p>
    </section>
  );
}
  ...
  return (
    <article className="rounded-2xl border border-carliz-pink/20 bg-white p-4 shadow-sm">
      <div className="mb-3 flex items-start justify-between gap-2">
        ...
        {badge ? <span className="rounded-full bg-carliz-pink px-2 py-1 text-xs font-medium text-white">{badge}</span> : null}
      </div>
  ...
  return (
    <section className="mt-8 rounded-2xl bg-white p-5 shadow-md">
      ...
      {selectedProducts.length ? (
        <>
          <ul className="mt-4 space-y-2 text-sm text-slate-700">
            {selectedProducts.map((item) => (
              <li key={item.id}>
                {item.qty}x {item.name} — <strong>{BRL.format(item.qty * item.price)}</strong>
              </li>
            ))}
          </ul>
        ...
        <p className="mt-4 text-slate-600">Nenhum item selecionado ainda.</p>
      )}
...
export default function App() {
  ...
  const selectedProducts = useMemo(
    () => allProducts.filter((item) => (cart[item.id] || 0) > 0).map((item) => ({ ...item, qty: cart[item.id] || 0 })),
    [allProducts, cart],
  );
  ...
  if (isLoading || !data) {
    return (
      <main className="mx-auto max-w-4xl p-8">
        <p className="text-center text-carliz-dark">Carregando cardápio...</p>
      </main>
    );
  }
    ...
    <main className="mx-auto min-h-screen w-full max-w-5xl px-4 py-8 md:px-8">
      ...
      <section className="mt-6">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {data.regularProducts.map((product) => (
            <ProductCard
              key={product.id}
              product={product}
          ...
          {showEaster
            ? data.easterProducts.map((product) => (
                <ProductCard
                  key={product.id}
                  product={product}
                  qty={cart[product.id] || 0}
