...
const ConfeitariaEmAcaoSection = lazy(() =>
  import('../sections/ConfeitariaEmAcaoSection').then((module) => ({
    default: module.ConfeitariaEmAcaoSection,
  }))
);
...
const HOME_SEO = {
  ...
  description:
    'Encomende brigadeiros, doces finos e cardápio especial da Carliz Doces. Atendimento rápido no WhatsApp e produção artesanal em São Paulo.',
...
export function HomePage({ skipIntroCurtain = false }) {
  ...
  const [maxMenuShowcasePrice, setMaxMenuShowcasePrice] = useState(() =>
    Math.max(
      ...seasonalProducts.filter((item) => isEasterMenuProduct(item)).map((item) => item.price),
      0
    )
  );
  const [maxOrderShowcasePrice, setMaxOrderShowcasePrice] = useState(() =>
    Math.max(
      ...seasonalProducts.filter((item) => isCandyOrderProduct(item)).map((item) => item.price),
      0
    )
  );
  ...
  const easterMenuProducts = useMemo(
    () => seasonalProducts.filter((item) => isEasterMenuProduct(item)),
    []
  );
  const candyOrderProducts = useMemo(
    () => seasonalProducts.filter((item) => isCandyOrderProduct(item)),
    []
  );
  ...
  const selectedMenuShowcaseProduct =
    menuShowcaseProducts[menuProductStep] ?? menuShowcaseProducts[0] ?? null;
  const selectedOrderShowcaseProduct =
    orderShowcaseProducts[orderProductStep] ?? orderShowcaseProducts[0] ?? null;
  ...
  const handleShareProduct = async (item) => {
    ...
    try {
      ...
      setSnackbar({
        open: true,
        message: 'Produto compartilhado com sucesso!',
        severity: 'success',
      });
    } catch {
      setSnackbar({
        open: true,
        message: 'Não foi possível compartilhar agora.',
        severity: 'error',
      });
    }
  };
  ...
  const handleFavoriteProduct = async (item) => {
    ...
    try {
      ...
      setSnackbar({
        open: true,
        message: !wasFavorite
          ? `${item.name} recebeu +1 coração!`
          : `Você removeu seu coração de ${item.name}.`,
        severity: 'success',
      });
    } catch {
      setFavoriteProductIds((currentFavorites) => {
  ...
  const handleRateProduct = async (item, rating, sectionType) => {
    const submitRating =
      sectionType === 'pedidos-de-doces' ? submitCandyRating : submitEasterRating;
    ...
    if (result.removed) {
      setSnackbar({
        open: true,
        message: `Sua avaliação de ${item.name} foi removida.`,
        severity: 'info',
      });
      return;
    }
    ...
    if (result.isRemote) {
      setSnackbar({
        open: true,
        message: `Avaliação enviada! Obrigado por avaliar ${item.name}.`,
        severity: 'success',
      });
      return;
    }
    ...
    setSnackbar({
      open: true,
      message: `Avaliação salva neste dispositivo para ${item.name}.`,
      severity: 'info',
    });
  };
  ...
  const handleContactSubmit = (event) => {
    ...
    if (!name || !message) {
      setSnackbar({
        open: true,
        message: 'Preencha nome e mensagem para enviar no WhatsApp.',
        severity: 'warning',
      });
      return;
    }
    ...
    setSnackbar({
      open: true,
      message: 'Mensagem preparada! Continue o envio no WhatsApp.',
      severity: 'success',
    });
  };
  ...
  const handleContactEmailSubmit = () => {
    ...
    if (!name || !message) {
      setSnackbar({
        open: true,
        message: 'Preencha nome e mensagem para enviar no e-mail.',
        severity: 'warning',
      });
      return;
    }
  ...
  useEffect(() => {
    const imageUrls = Array.from(
      new Set(seasonalProducts.map((product) => product.image).filter(Boolean))
    );
  ...
  useEffect(() => {
    ...
    const bootstrapLikes = async () => {
      try {
        const summary = await requestLikesSummary(
          deviceId,
        ...
        setFavoriteProductIds(() =>
          seasonalProducts
            .filter((product) => Boolean(likedByCurrentUserById[product.id]))
      ...
      <Box id="top" className="site-wrapper">
              ...
              <Box className="intro-clown-wrap">
                ...
                >
                  <Box
                    ...
                    alt="Palhaço saltando de alegria"
                    className="intro-clown"
                  />
                </motion.div>
                <motion.div
                ...
                >
                  <Box
                    component="img"
                    src="/images/tela-apresentacao/palhaco.png"
                    alt="Palhaço fazendo cambalhota"
                    className="intro-clown"
                  />
                </motion.div>
                <Box component="p" className="intro-message">
                  Respeeeitável púúúúblico! 🎪✨ Com muita alegria, muito brilho e uma pitadinha de
                  travessura, apresentaaamos… Carliz Doces! 🍭🍬🤡
                </Box>
              </Box>
            </Box>
          </Box>
        ...
        <main>
          <Container maxWidth="lg" className="page-container section-stack">
            ...
            <MotionDiv
              {...revealAnimation}
              transition={{ ...revealAnimation.transition, delay: 0.05 }}
            >
            ...
            <MotionDiv
              {...revealAnimation}
              transition={{ ...revealAnimation.transition, delay: 0.12 }}
            >
            ...
            <MotionDiv
              {...revealAnimation}
              transition={{ ...revealAnimation.transition, delay: 0.14 }}
            >
            ...
            <MotionDiv
              {...revealAnimation}
              transition={{ ...revealAnimation.transition, delay: 0.15 }}
            >
              <ShowcaseSection
                BRL={BRL}
                ...
                onRateProduct={(item, rating) =>
                  handleRateProduct(item, rating, 'cardapio-de-pascoa')
                }
                isGlobalRatingsActive={isEasterGlobalRatingsActive}
                disablePrevAtLast
              />
            ...
            <MotionDiv
              {...revealAnimation}
              transition={{ ...revealAnimation.transition, delay: 0.18 }}
            >
            ...
            <MotionDiv
              {...revealAnimation}
              transition={{ ...revealAnimation.transition, delay: 0.19 }}
            >
              <ShowcaseSection
                BRL={BRL}
                ...
                onRateProduct={(item, rating) =>
                  handleRateProduct(item, rating, 'pedidos-de-doces')
                }
                isGlobalRatingsActive={isCandyGlobalRatingsActive}
                disablePrevAtLast
              />
            ...
            <MotionDiv
              {...revealAnimation}
              transition={{ ...revealAnimation.transition, delay: 0.2 }}
            >
            ...
            <MotionDiv
              {...revealAnimation}
              transition={{ ...revealAnimation.transition, delay: 0.22 }}
            >
              <OrderSection
                BRL={BRL}
            ...
            <MotionDiv
              {...revealAnimation}
              transition={{ ...revealAnimation.transition, delay: 0.23 }}
            >
            ...
            <MotionDiv
              {...revealAnimation}
              transition={{ ...revealAnimation.transition, delay: 0.24 }}
            >
              <Suspense
                fallback={<Alert severity="info">Carregando experiência interativa…</Alert>}
              >
            ...
            <MotionDiv
              {...revealAnimation}
              transition={{ ...revealAnimation.transition, delay: 0.25 }}
            >
            ...
            <MotionDiv
              {...revealAnimation}
              transition={{ ...revealAnimation.transition, delay: 0.27 }}
            >
              <LocationSection />
            </MotionDiv>
            ...
            <Suspense
              fallback={
                <Container>
                  <Alert severity="info">Carregando seção...</Alert>
                </Container>
              }
            >
              <Container disableGutters className="section-stack">
                <MotionDiv {...revealAnimation}>
                  <SectionDivider label="Depoimentos" sectionId="depoimentos" />
                </MotionDiv>
                <MotionDiv {...revealAnimation}>
                  <DepoimentosSection testimonials={communityTestimonials} />
                </MotionDiv>
                <MotionDiv {...revealAnimation}>
                  <SectionDivider label="Novidades" sectionId="novidades" />
                </MotionDiv>
                <MotionDiv {...revealAnimation}>
                  <NovidadesSection updates={updates} announcementChannels={announcementChannels} />
                </MotionDiv>
                <MotionDiv {...revealAnimation}>
                  <SectionDivider label="Contato" sectionId="contato" />
                </MotionDiv>
                <MotionDiv {...revealAnimation}>
                  <ContatoSection
                ...
                <MotionDiv {...revealAnimation}>
                  <SectionDivider label="Instagram" sectionId="instagram" />
                </MotionDiv>
                <MotionDiv {...revealAnimation}>
        ...
        >
          ...
          >
            ...
            <DialogTitle
              ...
              sx={{ p: 0, pr: 5, fontWeight: 800, color: '#8d285a' }}
            >
              Nosso cantinho doce
            </DialogTitle>
            <Typography variant="body2" sx={{ mt: 1, color: 'rgba(111, 39, 71, 0.8)' }}>
              Assista ao nosso vídeo demonstrativo e veja como preparamos nossos doces com qualidade
              e segurança.
            </Typography>
            <Button
              ...
              sx={{
                ...
                '@keyframes googleReviewBlink': {
                  ...
                  '50%': {
                    ...
                    boxShadow:
                      '0 0 0 6px rgba(255, 0, 119, 0.22), 0 14px 24px rgba(255, 0, 119, 0.5)',
        ...
        >
          ...
          <DialogContent>
            <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
              Selecione seu provedor de e-mail preferido para continuar com a mensagem já
              preenchida.
            </Typography>
            <Stack spacing={1}>
              <Button variant="contained" onClick={() => handleEmailProviderSelect('gmail')}>
                Gmail
              </Button>
              <Button variant="contained" onClick={() => handleEmailProviderSelect('outlook')}>
                Outlook
              </Button>
              <Button variant="contained" onClick={() => handleEmailProviderSelect('yahoo')}>
                Yahoo
              </Button>
              <Button variant="outlined" onClick={() => handleEmailProviderSelect('other')}>
                Outros aplicativos
              </Button>
            </Stack>
          </DialogContent>
