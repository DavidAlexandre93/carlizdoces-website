...
export function BakerStorySection() {
  ...
  useEffect(() => {
    const update = () => {
      ...
      setActiveStep(
        Math.min(bakingSteps.length - 1, Math.floor(nextProgress * bakingSteps.length))
      );
    };
      ...
      <Box className="baker-story-layout">
        <Box className="baker-story-stage-wrap">
          <Box className="baker-story-stage">
            <MotionDiv
              className="baker-chef"
              animate={{ y: `${-22 * progress}px` }}
              transition={{ duration: 0.3 }}
            >
              👩‍🍳
            </MotionDiv>
            <MotionDiv
              className="baker-cream-line"
              animate={{ scaleX: 0.2 + 0.8 * progress }}
              transition={{ duration: 0.3 }}
            />
            <MotionDiv
              className="baker-cake"
              animate={{ scale: 0.88 + 0.18 * progress }}
              transition={{ duration: 0.3 }}
            >
              <span role="img" aria-label="bolo">
                🎂
              </span>
            </MotionDiv>
          </Box>
        ...
        <Stack spacing={2} className="baker-steps-list">
          {bakingSteps.map((step, index) => (
            <Box
              key={step.label}
              className={`baker-step-card ${index === activeStep ? 'is-active' : ''}`}
            >
