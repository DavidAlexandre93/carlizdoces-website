...
export default function DepoimentosSection({ testimonials }) {
  ...
  const disqusConfig = useMemo(() => {
    const pageUrl =
      typeof window !== 'undefined'
        ? `${window.location.origin}${window.location.pathname}#depoimentos`
        : 'https://carlizdoces.com/#depoimentos';
  ...
  useGSAP(
    (context) => {
      gsap.from(
        '.testimonial-alert',
        { x: -18, opacity: 0, duration: 0.6, stagger: 0.08, ease: 'power2.out' },
        context.scope
      );
      gsap.from(
        '.testimonials-disqus',
        { y: 18, opacity: 0, duration: 0.7, delay: 0.15, ease: 'power3.out' },
        context.scope
      );
    },
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      const disqusCountScript = document.querySelector(
