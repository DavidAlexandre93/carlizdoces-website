        ...
        {selectedShowcaseProduct ? (
          <Paper className="showcase-card" sx={{ p: { xs: 1.25, sm: 1.5 } }}>
            <Box className="showcase-image-stage">
              <SwipeableViews axis={theme.direction === 'rtl' ? 'x-reverse' : 'x'} index={activeProductStep} onChangeIndex={setActiveProductStep} enableMouseEvents>
                {visibleShowcaseProducts.map((product) => (
                  <img key={product.id} className="showcase-image" src={product.image} alt={product.name} loading="eager" decoding="async" />
                ))}
              </SwipeableViews>
            ...
            <Box sx={{ mt: 1 }}>
              ...
              <Rating precision={1} value={ratingStats?.userRating ?? 0} onChange={(_event, nextValue) => { if (nextValue) onRateProduct(selectedShowcaseProduct, nextValue) }} />
            ...
            <Box sx={{ mt: 1, display: 'flex', gap: 1 }}>
              ...
              <Box className="favorite-heart-button" sx={{ display: 'inline-flex' }} data-favorite-count={favoriteCounts[selectedShowcaseProduct.id] ?? 0} data-is-favorite={favoriteProductIds.includes(selectedShowcaseProduct.id)} data-has-favorite-handler={typeof onFavoriteProduct === 'function'}>
